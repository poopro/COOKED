...
from ..services.llm_cost_estimator import (
    build_cost_estimate_payload,
...
logger = get_logger('cooked.api.simulation')
    ...
    请求（JSON）：
        {
            ...
            "graph_id": "cooked_xxxx",    // 可选，如不提供则从project获取
...
@simulation_bp.route('/cost-estimate', methods=['GET'])
def simulation_cost_estimate():
    """
    根據當前 .env 模型（或可選 model 覆寫）與 simulation_config.json，
    粗估本次 OASIS 主循環 LLM 費用區間，並附 OpenRouter 上較便宜／較高品質模型建議。

    Query:
        simulation_id: 必填
        platform: parallel | twitter | reddit，預設 parallel
        max_rounds: 可選，與開始模擬時截斷輪數一致時較準
        graph_memory: true|false，是否加計 Zep 圖譜回寫額度
        model: 可選，OpenRouter id（如 openai/gpt-4o-mini）覆寫僅用於試算
    """
    try:
        simulation_id = (request.args.get("simulation_id") or "").strip()
        if not simulation_id:
            return jsonify({"success": False, "error": "請提供 simulation_id"}), 400

        platform = (request.args.get("platform") or "parallel").strip().lower()
        if platform not in ("parallel", "twitter", "reddit"):
            return jsonify({"success": False, "error": "platform 必須為 parallel / twitter / reddit"}), 400

        max_rounds_raw = request.args.get("max_rounds")
        max_rounds = None
        if max_rounds_raw is not None and str(max_rounds_raw).strip() != "":
            try:
                max_rounds = int(max_rounds_raw)
            except ValueError:
                return jsonify({"success": False, "error": "max_rounds 必須為整數"}), 400
            if max_rounds <= 0:
                max_rounds = None

        graph_memory = request.args.get("graph_memory", "false").lower() in ("1", "true", "yes")

        config = load_simulation_config_from_disk(simulation_id)
        model_override = (request.args.get("model") or "").strip()
        resolved_model = model_override or Config.LLM_MODEL_NAME or config.get("llm_model") or "openai/gpt-4o-mini"

        payload = build_cost_estimate_payload(
            config=config,
        ...
        def run_prepare():
            try:
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    stage_names = {
                        "reading": "讀取圖譜實體",
                        "generating_profiles": "產生 Agent 人設",
                        "generating_config": "產生模擬設定",
                        "copying_scripts": "準備模擬腳本"
                    }
...
# ============== 购买意愿评估接口（GAP-style + Info Plaza 訊號）==============


@simulation_bp.route('/<simulation_id>/purchase-intent', methods=['POST'])
def run_purchase_intent_evaluation(simulation_id: str):
    """
    對指定模擬跑「購買意願評估」。

    每位 agent 會被 LLM 分析：
      1. 隱藏的 5 維度心理 meter（敏感度）
      2. 是否屬於目標客群（target / non-target）
      3. 結合產品 + 廣告 + 內部 meter + Info Plaza 模擬行為 → BUY/REJECT

    Body (JSON):
        {
            "product_desc": "string,必填",
            "ad_copy": "string,必填",
            "target_audience_criteria": "string,必填，描述誰才是目標客群",
            "sample_size": 12        // 可選，預設 12，最多取 personas 全部
        }

    Returns:
        完整的評估結果（同時也存到 simulation 目錄的 purchase_intent.json）
    """
    try:
        body = request.get_json(silent=True) or {}
        product_desc = (body.get("product_desc") or "").strip()
        ad_copy = (body.get("ad_copy") or "").strip()
        ta_criteria = (body.get("target_audience_criteria") or "").strip()
        sample_size = body.get("sample_size")

        if not product_desc:
            return jsonify({"success": False, "error": "product_desc 不可為空"}), 400
        if not ad_copy:
            return jsonify({"success": False, "error": "ad_copy 不可為空"}), 400
        if not ta_criteria:
            return jsonify({
                "success": False,
                "error": "target_audience_criteria 不可為空（請描述目標客群）"
            }), 400

        evaluator = PurchaseIntentEvaluator()
        payload = evaluator.run(
            simulation_id=simulation_id,
            product_desc=product_desc,
            ad_copy=ad_copy,
            target_audience_criteria=ta_criteria,
...
@simulation_bp.route('/<simulation_id>/purchase-intent', methods=['GET'])
def get_purchase_intent_evaluation(simulation_id: str):
    """讀取最近一次的購買意願評估結果（如果有）。"""
    try:
        payload = PurchaseIntentEvaluator.load_saved_result(simulation_id)
        if payload is None:
            return jsonify({
                "success": True,
                "data": None,
        ...
        logger.error(f"讀取購買意願評估失敗: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
