<template>
  <div class="interaction-panel">
          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '與世界中任意個體對話' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">選擇對話對象</div>
                <div 
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || '未知職業' }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            >
              ...
              <span>發送問卷調查到世界中</span>
            </button>
            <button 
              class="tab-pill purchase-pill"
              @click="openPurchasePanel"
              :title="'結合 5 維內部 meter + Info Plaza 模擬訊號 → 預測買 / 不買 + TA / 路人對比'"
            >
              <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge 深度歸因</div>
                    <div class="tool-desc">對齊現實世界種子資料與模擬環境狀態，結合 Global／Local Memory 機制，提供跨時空的深度歸因分析</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch 全景追蹤</div>
                    <div class="tool-desc">基於圖結構的廣度遍歷演算法，重構事件傳播路徑，捕獲全量資訊流動的拓撲結構</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch 快速檢索</div>
                    <div class="tool-desc">基於 GraphRAG 的即時查詢介面，優化索引效率，用於快速提取具體的節點屬性與離散事實</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent 虛擬訪談</div>
                    <div class="tool-desc">自主式訪談，能夠並行與模擬世界中的個體進行多輪對話，採集非結構化的觀點資料與心理狀態</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || '未知職業' }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">簡介</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? '與 Report Agent 對話，深入了解報告內容' : '與模擬個體對話，了解他們的觀點' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">選擇調查對象</span>
                <span class="selection-count">已選 {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || '未知職業' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">全選</button>
            ...
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">問卷問題</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="輸入您想問所有已選對象的問題..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">調查結果</span>
              <span class="results-count">{{ surveyResults.length }} 則回覆</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || '未知職業' }}</span>
                  </div>
                </div>
    ...
    <!-- 購買意願評估 - 全螢幕 overlay (從 Interactive Tools 工具列觸發) -->
    <PurchaseIntentPanel
      v-if="showPurchasePanel"
...
import PurchaseIntentPanel from './PurchaseIntentPanel.vue'
...
// === 購買意願評估 (Purchase Intent) ===
// 此面板以全螢幕 overlay 出現，關閉後回到原分頁
const showPurchasePanel = ref(false)
const openPurchasePanel = () => {
  if (!props.simulationId) {
    alert('尚未載入模擬資料，無法評估購買意願')
    return
  }
  showAgentDropdown.value = false
...
const sendToReportAgent = async (message) => {
  addLog(`向 Report Agent 發送: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || '無回應',
      timestamp: new Date().toISOString()
    })
    addLog('Report Agent 已回复')
  } else {
    throw new Error(res.error || '請求失敗')
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('請先選擇一個模擬個體')
  }
  
  addLog(`向 ${selectedAgent.value.username} 發送: ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? '提問者' : '你'}：${msg.content}`)
      .join('\n')
    prompt = `以下是我們先前的對話：\n${historyContext}\n\n現在我的新問題是：${message}`
  }
...
const submitSurvey = async () => {
  ...
  addLog(`發送問卷給 ${selectedAgents.value.size} 個對象...`)
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || '無回應'
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容数组格式
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || '無回應'
          }
        }
      ...
      addLog(`收到 ${surveyResults.value.length} 則回覆`)
    } else {
      throw new Error(res.error || '請求失敗')
    }
  } catch (err) {
    addLog(`問卷發送失敗: ${err.message}`)
  } finally {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`載入了 ${profiles.value.length} 個模擬個體`)
    }
  } catch (err) {
    addLog(`載入模擬個體失敗: ${err.message}`)
  }
}
...
/* Left Panel - Report Style（與 Step4Report.vue 完全一致） */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
