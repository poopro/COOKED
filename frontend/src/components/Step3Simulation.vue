<template>
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          ...
          <div class="platform-stats">
            ...
            <span class="stat">
              <span class="stat-label">Elapsed Time <span class="stat-label-note">(模擬世界)</span></span>
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? '啟動中...' : '開始產生結果報告' }} 
    ...
    <!-- 實際運行時間（牆上時鐘）：避免誤以為當機 -->
    <div v-if="phase === 1 && wallElapsedFormatted" class="wall-clock-bar">
      <span class="wall-clock-label">實際已運行</span>
      <span class="wall-clock-value mono">{{ wallElapsedFormatted }}</span>
      <span class="wall-clock-hint">首輪常需載入 Agent 圖 + 大量 LLM，ROUND/ACTS 仍為 0 可能屬正常。</span>
    </div>

    <!-- 模擬失敗（runner_status === 'failed'）：紅色 banner -->
    <div v-if="phase === 3 && failedReason" class="failure-bar">
      <div class="failure-row">
        <span class="failure-icon">⚠</span>
        <span class="failure-title">模擬子程序崩潰</span>
        <span class="failure-reason">{{ failedReason }}</span>
      </div>
      <div class="failure-hint">
        多半是 Python venv 環境壞了。建議：
        <code class="inline-code">stop.bat</code> →
        刪除 <code class="inline-code">backend/.venv</code> →
        重新執行 <code class="inline-code">run.bat</code>。
        詳細錯誤請看 <code class="inline-code">backend/uploads/simulations/{{ simulationId }}/simulation.log</code>。
      </div>
    </div>
    ...
    <div class="main-content-area" ref="scrollContainer">
      ...
      <div class="timeline-feed">
        ...
        <div v-if="allActions.length === 0" class="waiting-state">
          ...
          <p v-if="phase === 1" class="waiting-explain">
            子程序正在執行 OASIS（Twitter + Reddit 並行）。每位 Agent 每輪需呼叫 LLM（並發約 30），Agent 數量多時首輪完成前可不寫入動作日誌。
            若超過約 90 分鐘仍為 0，請打開後端視窗或檢查 <code class="inline-code">backend/uploads/simulations/&lt;sim_id&gt;/simulation.log</code> 是否報錯。
          </p>
        </div>
      </div>
    </div>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // 自 Step2 傳入的最大輪數
  minutesPerRound: {
    type: Number,
    default: 30 // 默认每轮30分钟
  },
...
const phase = ref(0) // 0: 未開始, 1: 執行中, 2: 已完成, 3: 已失敗
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // 所有動作（增量累積）
const actionIds = ref(new Set()) // 用於去重的動作 ID 集合
const scrollContainer = ref(null)
// 运行中崩溃失败的原因（runner_status==='failed' 时填入）
const failedReason = ref(null)

// Computed
// 依時間順序顯示動作（最新的在最後，即底部）
...
/** 牆上時鐘：從後端 started_at 算起，與「模擬世界 Elapsed」不同 */
...
const wallElapsedFormatted = computed(() => {
  if (phase.value !== 1) return ''
  const started = runStatus.value?.started_at
  if (!started) return ''
  const t0 = new Date(started).getTime()
  if (Number.isNaN(t0)) return ''
  const sec = Math.max(0, Math.floor((wallClockNow.value - t0) / 1000))
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 強制重新開始
      enable_graph_memory_update: true  // 啟用動態圖譜更新
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`設定最大模擬輪數: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      ...
      startError.value = res.error || '啟動失敗'
      addLog(`✗ 啟動失敗: ${res.error || '未知錯誤'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ 啟動異常: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // === 優先：偵測後端是否已將 runner_status 設為 failed ===
      // 这种情况下子进程已崩溃，不能再傻傻地停留在「Running」
      if (data.runner_status === 'failed') {
        const reason = data.error || data.runner_error || data.failure_reason || '子行程異常結束，請檢查 simulation.log'
        if (failedReason.value !== reason) {
          failedReason.value = reason
          addLog(`✗ 模擬執行失敗：${reason}`)
          addLog('   提示：請開啟 backend/uploads/simulations/<sim_id>/simulation.log 查看完整堆疊。')
        }
        phase.value = 3
        stopPolling()
        emit('update-status', 'error')
      ...
      // 偵測模擬是否已完成（依 runner_status 或平台完成狀態判斷）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // 額外檢查：若後端尚未來得及更新 runner_status，但平台已回報完成
      // 透過偵測 twitter_completed 與 reddit_completed 狀態判斷
...
const checkPlatformsCompleted = (data) => {
  ...
  // 若至少有一個平台完成，檢查是否所有啟用的平台都完成
  // 透過 actions_count 判斷平台是否啟用（若 count > 0 或 running 曾為 true）
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ 報告產生任務已啟動: ${reportId}`)
      
      // 跳轉到報告頁面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ 啟動報告產生失敗: ${res.error || '未知錯誤'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ 啟動報告產生異常: ${err.message}`)
    isGeneratingReport.value = false
  }
}
