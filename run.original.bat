...
if exist "backend\.venv\.cooked_deps_ok" goto skip_dep_check
...
echo ok > "backend\.venv\.cooked_deps_ok"
goto step3
...
start "COOKED? Backend [5001]" cmd /k "chcp 65001 >nul && cd /d %~dp0backend && set FLASK_APP=app && set FLASK_DEBUG=True && echo === COOKED? Backend === && echo Port: 5001 && echo. && .venv\Scripts\python.exe -m flask run --port 5001 --host 0.0.0.0"
start "COOKED? Frontend [5173]" cmd /k "chcp 65001 >nul && cd /d %~dp0frontend && echo === COOKED? Frontend === && echo Port: 5173 && echo. && npm run dev"
...
echo         Make sure run.bat lives in the COOKED? project root,
...
del /F /Q "backend\.venv\.cooked_deps_ok" >nul 2>&1
goto end_pause
